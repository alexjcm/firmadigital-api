...
public class ClienteRestServicioDocumento {
    ...
    // FIXME: Externalize
    private static String REST_SERVICE_URL = "http://localhost:8080/servicio/documentos";

    public String obtenerDocumento(String token) {
        Client client = ClientBuilder.newClient();
        WebTarget target = client.target(REST_SERVICE_URL);

        String archivo = target.path("{token}").resolveTemplate("token", token).request(MediaType.TEXT_PLAIN)
                .get(String.class);
        return archivo;
    }

    public String guardarDocumento(String token, String archivo) {
        Client client = ClientBuilder.newClient();
        WebTarget target = client.target(REST_SERVICE_URL);

        String response = target.path("{token}").resolveTemplate("token", token).request(MediaType.TEXT_PLAIN)
